 ...
 *
 * Source resolution delegated to frl_schema_resolve_post_source() for
 * post_*-prefixed sources; everything else resolves via post meta.
 *
...
function frl_schema_builder_build_person_from_ref( int $ref_id, array $field_def ): ?array {
	...
	foreach ( $field_def as $sub_field => $source ) {
		...
		// Scalar source: delegate to shared resolver
		$value = frl_schema_resolve_post_source( $ref_id, $source, $post );
